...
class FlowerGameAdapter:
    ...
    def __init__(self, original_adapter):
        ...
        self.pending_keyboard_input = False  # 是否等待键盘输入
    ...
    async def _send_chat_tts_text_packets(self, text: str) -> bool:
        ...
        # 在问题状态下启用键盘输入
        if self.game_state.startswith("question_"):
    ...
    def handle_keyboard_input(self, char: str):
        """处理键盘输入"""
        if not self.pending_keyboard_input or self.is_playing_audio:
            return
            
        if char in ['1', '2', '3', '4', '5'] and self.game_state.startswith("question_"):
            logger.info(f"键盘输入选择: {char}")
            self.pending_keyboard_input = False
            # 直接处理选项，不通过语音识别
            asyncio.create_task(self._handle_keyboard_option(char))
    
    async def _handle_keyboard_option(self, option: str):
        """处理键盘选项输入"""
        ...
        logger.info(f"用户选择(键盘): {option}, 得分: {score}")
