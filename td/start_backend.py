...
from src.adapters.touchdesigner_adapter import TouchDesignerConnectionConfig
...
def main():
    ...
    parser.add_argument(
        "--td-ip", default="localhost", 
        help="TouchDesigner IP地址 (默认: localhost)"
    )
    parser.add_argument(
        "--td-port", type=int, default=7000, 
        help="TouchDesigner端口 (默认: 7000)"
    )
    parser.add_argument(
        "--listen-port", type=int, default=7001, 
        help="监听TouchDesigner音频的端口 (默认: 7001)"
    )
    parser.add_argument(
        "--app-id", default=VOLCENGINE_APP_ID,
        help="火山引擎应用ID"
    )
    parser.add_argument(
        "--access-token", default=VOLCENGINE_ACCESS_TOKEN,
        help="火山引擎访问令牌"
    )
    parser.add_argument(
        "--use-pcm", action="store_true", default=True,
